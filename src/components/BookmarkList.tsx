...
export default function BookmarkList({
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-center gap-2 text-xs text-green-600 font-medium mb-8 bg-green-50 rounded-lg py-2 px-4 inline-flex">
      ...
      {bookmarks?.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {bookmarks?.map((item) => (
            <div
              key={item.id}
              className="group bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md hover:border-gray-300 transition-all overflow-hidden p-5 flex flex-col"
            >
              {/* Link Section */}
              <a
                href={item.url}
                ...
                className="text-blue-600 font-semibold text-base mb-3 hover:text-blue-700 hover:underline line-clamp-2 break-words"
              >
                {item.title}
              </a>
